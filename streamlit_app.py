...
def _queue_selected_session(session_id: str) -> None:
    """Defer widget-backed session changes until before the selector is created."""
    st.session_state["_pending_selected_session_id"] = session_id


def _reset_live_monitor_state() -> None:
    st.session_state["_hist_key"] = None
...
def _uploaded_file_signature(uploaded_file) -> tuple[str, int | None]:
    ...
    return getattr(uploaded_file, "name", "upload.mp4"), getattr(buffer, "nbytes", None)


def _resolve_uploaded_video(uploaded_file, state_key: str) -> str | None:
    ...
    if cached and cached.get("signature") == signature:
        ...
        if cached_path and Path(cached_path).exists():
            st.session_state["uploaded_video_path"] = cached_path
            return cached_path

    saved = save_uploaded_video(uploaded_file)
...
def _analysis_extra_args(fast_mode_enabled: bool) -> list[str]:
    return ["--no-output-video"] if fast_mode_enabled else []


def _job_launch_timestamp(job_state: dict | None):
    if not job_state:
        return None
    return parse_iso_timestamp(job_state.get("launched_at"))


def _job_compatible_records(session_records: list[dict], active_sport: str | None, job_state: dict | None) -> list[dict]:
    if not _is_running(job_state):
        return session_records

    launched_at = _job_launch_timestamp(job_state)
    if launched_at is None:
        return session_records

    allowed_earliest = launched_at - timedelta(seconds=3)
    job_sport = str(job_state.get("sport") or active_sport or "").lower()
    compatible: list[dict] = []
    for record in session_records:
        record_sport = str(record.get("sport", "")).lower()
        if job_sport and record_sport and record_sport != job_sport:
            continue
        record_timestamp = (
            parse_iso_timestamp(record.get("session_started_at"))
            or parse_iso_timestamp(record.get("last_updated_at"))
        )
        if record_timestamp is None or record_timestamp < allowed_earliest:
            continue
        compatible.append(record)
    return compatible


def _build_pending_monitor_payload(active_sport: str | None, job_state: dict | None) -> dict:
    data = load_stats(session_records=[], allow_legacy_fallback=False)
    sport_name = str((job_state or {}).get("sport") or active_sport or data.get("sport") or "tennis").lower()
    try:
    ...
    launched_at = (job_state or {}).get("launched_at")
    source_type = (job_state or {}).get("source_type") or "file"
    source_value = (job_state or {}).get("video_path")
    source_label = f"{source_type.title()}: {source_value}" if source_value else source_type.title()

    data.update({
        "status": "starting" if _is_running(job_state) else "idle",
        "sport": sport_name,
        "session_id": None,
        "session_started_at": launched_at,
        "last_updated_at": launched_at,
        "source_video": source_value or data.get("source_video"),
        "source": {
            "type": source_type,
            "label": source_label,
            "uri": source_value,
        ...
        "sport_profile": {
            "display_name": sport_profile.display_name,
            "equipment_name": sport_profile.equipment_name,
            "ball_name": sport_profile.ball_name,
            "ball_like_object_name": sport_profile.ball_like_object_name,
        ...
        "notes": ["Waiting for the new analysis session to produce fresh frames..."],
    })
    return data
...
def render_analysis_progress(data: dict, job_state, running: bool) -> None:
    ...
    is_live_source = source_type in ("rtsp",)
...
def _load_monitor_snapshot(active_sport: str | None, selected_session_id: str, job_state: dict | None = None) -> dict:
    session_records = discover_session_records()
    workspace_records = [
        record for record in session_records
        if not active_sport or str(record.get("sport", "")).lower() == str(active_sport).lower()
    ]
    candidate_records = workspace_records if active_sport else session_records
    load_session_id = selected_session_id
    if load_session_id == LATEST_SESSION_OPTION and active_sport:
        fresh_records = _job_compatible_records(workspace_records, active_sport, job_state)
        if _is_running(job_state) and not fresh_records:
            data = _build_pending_monitor_payload(active_sport, job_state)
            summary = data.get("summary", {})
            ...
            return {
                ...
                "data_freshness": freshness_label(current_data_age_seconds(data), str(data.get("status", "unknown"))),
                "data_age_seconds": current_data_age_seconds(data),
                "history": history,
                "all_notes": list(data.get("notes", [])),
                "performance_metrics": data.get("performance_metrics", {}),
                "preview_path": None,
                "preview_bytes": None,
                "display_video_path": None,
                "display_video_label": "Video",
            }
        candidate_records = fresh_records or workspace_records
        load_session_id = _latest_session_id_for_sport(active_sport, candidate_records)

    data = load_stats(
        selected_session_id=load_session_id,
        session_records=candidate_records,
        allow_legacy_fallback=not bool(active_sport),
    )
    ...
    st.session_state["live_frame_index"] = int(data.get("frame_index", 0) or 0)
    st.session_state["live_metrics"] = {
        "players": summary.get("players_detected"),
        "ball_speed": summary.get("ball_speed_px_per_sec"),
        "swings": summary.get("active_swing_count"),
        "impact_power": summary.get("impact_power_score"),
        "coaching_tips": summary.get("recommendation_count"),
    }
    ...
    return {
        ...
        "display_video_label": display_video_label,
    }


def _render_monitor_dynamic_sections(
    snapshot: dict,
    job_state,
    running: bool,
    ...
    display_video_path = snapshot["display_video_path"]
    display_video_label = snapshot["display_video_label"]
    all_notes = snapshot["all_notes"]

    with placeholders["progress"].container():
        render_analysis_progress(data, job_state, running)

    with placeholders["frame"].container():
        if preview_bytes:
    ...
    with placeholders["metrics"].container():
        ...
        metric_cols[0].metric("Players", _metric_val(summary.get("players_detected"), 0))
        metric_cols[1].metric("Ball Speed", _metric_val(summary.get("ball_speed_px_per_sec"), 1, " px/s", zero_is_empty=True))
        metric_cols[2].metric("Swings", _metric_val(summary.get("active_swing_count"), 0))
        metric_cols[3].metric("Impact Power", _metric_val(summary.get("impact_power_score"), 1, "", zero_is_empty=True))
        metric_cols[4].metric("Coaching Tips", _metric_val(summary.get("recommendation_count"), 0))

    with placeholders["charts"].container():
    ...
    with placeholders["video"].container():
...
def render_sport_hub(session_records: list[dict], job_state: dict | None) -> None:
    ...
    for col, sport in zip(cols, _PRIMARY_SPORTS):
        ...
        with col:
            ...
            if sport == "tennis":
                if st.button(
                    f"Open {profile.display_name}",
                    key=f"open_sport_{sport}",
                    ...
                    _queue_selected_session(LATEST_SESSION_OPTION)
                    st.session_state["app_page"] = "Analyze"
                    st.rerun()
            else:
                st.button(
                    f"{profile.display_name} (Coming Soon)",
                    key=f"open_sport_{sport}",
                    use_container_width=True,
                    disabled=True,
                )
...
def render_analyze_page(job_state, session_records: list) -> None:
    ...
    _VALID_MODES = ["Upload Video", "Live Camera", "Saved Video", "Demo", "Webcam", "Live Stream"]
    ...
    if source_mode == "Upload Video":
        ...
        source_value = _resolve_uploaded_video(uploaded, "_analyze_upload_cache")
        if source_value:
            st.success(f"Saved: {Path(source_value).name}")
        if not source_value:
            can_start = False
            st.caption("Upload a video file to continue.")

    elif source_mode == "Live Camera":
        camera_input = st.camera_input("Start Live Camera")
        ...
        if camera_input is not None:
            ...
            can_start = False
            st.caption("Capture a frame to continue.")
    ...
    # Step 3 — Processing
    st.markdown('<p class="step-label">Step 3 &nbsp; Processing</p>', unsafe_allow_html=True)
    fast_mode = st.checkbox(
        "Fast mode: skip processed video export for quicker analysis",
        value=True,
        help="Recommended for uploads when you want the results and risk alerts faster.",
        key="analyze_fast_mode",
    )
    if fast_mode:
        st.caption("Fast mode is on, so the dashboard will show live preview frames and the original clip instead of waiting for a processed video file.")
    ...
    if st.button(
        ...
        _reset_live_monitor_state()
        launch_analysis_process(
            source_value,
            selected_sport,
            source_type=source_type,
            cwd=Path.cwd(),
            extra_args=_analysis_extra_args(fast_mode),
        )
        _queue_selected_session(LATEST_SESSION_OPTION)
...
def render_analyze_page_v2(job_state, session_records: list) -> None:
    ...
    if source_mode == "Upload Video":
        ...
        source_value = _resolve_uploaded_video(uploaded, "_analyze_v2_upload_cache")
        if source_value:
            st.success(f"Saved: {Path(source_value).name}")
        if not source_value:
            can_start = False
    ...
    st.markdown('<p class="step-label">Step 4 &nbsp; Processing</p>', unsafe_allow_html=True)
    fast_mode = st.checkbox(
        "Fast mode: skip processed video export for quicker analysis",
        value=True,
        help="Recommended for uploads when you want the results and risk alerts faster.",
        key="analyze_v2_fast_mode",
    )
    if fast_mode:
        st.caption("Fast mode is on, so the dashboard will show live preview frames and the original clip instead of waiting for a processed video file.")

    st.markdown('<p class="step-label">Step 5 &nbsp; Start</p>', unsafe_allow_html=True)
    if st.button(
        "Start Analysis" if not running else "Stop current job first",
        use_container_width=True,
        disabled=(running or not can_start),
...
def render_monitor_page(
    ...
    initial_preview_bytes = None
    if preview_path and preview_path.exists():
        try:
            initial_preview_bytes = preview_path.read_bytes()
        except OSError:
            initial_preview_bytes = None
    snapshot = {
        "data": data,
        ...
        "preview_path": preview_path,
        "preview_bytes": initial_preview_bytes,
        "display_video_path": display_video_path,
        "display_video_label": display_video_label,
    }
    _render_monitor_dynamic_sections(snapshot, job_state, running, placeholders)

    if auto_refresh and running:
        refresh_seconds = max(1, int(st.session_state.get("refresh_interval_seconds", 2)))
        tick_seconds = min(0.2, max(0.05, refresh_seconds / 10))
        tick_count = max(1, int(refresh_seconds / tick_seconds))
        for _ in range(tick_count):
            time.sleep(tick_seconds)
            job_state = refresh_job_state(load_job_state())
            running = _is_running(job_state)
            snapshot = _load_monitor_snapshot(active_sport, selected_session_id, job_state)
            _render_monitor_dynamic_sections(snapshot, job_state, running, placeholders)
            if not running:
                break
        if running:
...
def render_health_tab(
    ...
    fall_count = summary.get("fall_alerts", 0) or pose_summary.get("confirmed_fall_count", 0)
    session_injury_frames = int(summary.get("session_injury_risk_frames", 0) or 0)
    session_fall_frames = int(summary.get("session_fall_alert_frames", 0) or 0)
    flags = list(primary_posture.get("injury_risk_flags", []))
    injury_snippet_keys = [key for key in snippets if str(key).startswith("injury_risk_player_")]
    fall_snippet_keys = [key for key in snippets if str(key).startswith("fall_detected_player_")]
    injury_clip_count = sum(len(snippets.get(key, [])) for key in injury_snippet_keys)
    fall_clip_count = sum(len(snippets.get(key, [])) for key in fall_snippet_keys)
    injury_review_frames = [
        frame for frame in bad_frames
        if "injury" in str(frame.get("reason", "")).lower()
    ]
    fall_review_frames = [
        frame for frame in bad_frames
        if "fall detected" in str(frame.get("reason", "")).lower()
    ]
    session_risk_count = int(injury_count or 0) + int(fall_count or 0) + len(flags) + injury_clip_count + fall_clip_count + len(injury_review_frames) + len(fall_review_frames) + session_injury_frames + session_fall_frames
    activity_vals = _series_values(performance_metrics.get("frame_series", []), "primary_activity_score")

    st.markdown("#### Health Overview")
    st.markdown("#### Health Overview")
    if session_risk_count > 0:
        st.warning("Session-level injury evidence was found. The final frame may still look safe if the risk happened earlier in the video.")
...
# =========================
    ...
    if injury_count > 0:
      risk_alerts.append("⚠ Injury Risk Detected")

    if len(flags) > 0:
      risk_alerts.append("⚠ Biomechanics Warning Detected")

    if posture_score and posture_score < 60:
      risk_alerts.append("⚠ Poor Posture Stability")

    if risk_alerts:
      for alert in risk_alerts:
        st.error(alert)
    else:
       st.success("✅ No critical live risks detected")
    ...
    c3.metric("Risk Flags", session_risk_count)
    c4.metric("Falls", session_fall_frames or fall_count or len(fall_review_frames))
    # ===== SESSION COMPARISON =====
    if "session_history" not in st.session_state:
        st.session_state["session_history"] = []

    current_session = {
        "posture": posture_score or 0,
        "risk": session_risk_count,
    }

    st.session_state["session_history"].append(current_session)

    if len(st.session_state["session_history"]) > 1:
        previous = st.session_state["session_history"][-2]
        current = st.session_state["session_history"][-1]

        posture_diff = current["posture"] - previous["posture"]

        if posture_diff > 0:
            st.success(f"Posture improved by {posture_diff:.1f}")
        elif posture_diff < 0:
            st.warning(f"Posture reduced by {abs(posture_diff):.1f}")
    ...
    with right:
        ...
        if session_fall_frames:
            st.error(f"{session_fall_frames} fall-positive frame{'s' if session_fall_frames != 1 else ''} detected in this session.")
        if fall_clip_count:
            st.error(f"{fall_clip_count} fall clip{'s' if fall_clip_count != 1 else ''} saved for review.")
        if fall_review_frames:
            st.error(f"{len(fall_review_frames)} fall review frame{'s' if len(fall_review_frames) != 1 else ''} saved.")
        if injury_clip_count:
            st.warning(f"{injury_clip_count} injury-risk clip{'s' if injury_clip_count != 1 else ''} detected across the session.")
        if injury_review_frames:
            st.warning(f"{len(injury_review_frames)} injury review frame{'s' if len(injury_review_frames) != 1 else ''} saved.")
        if fall_count:
            st.error(f"{fall_count} player fall alert{'s' if fall_count != 1 else ''} detected in the latest analysed frame.")
        ...
        elif not injury_clip_count and not injury_review_frames:
        ...
        priority_frames = injury_review_frames[:3] or bad_frames[:3]
        if priority_frames:
...
def render_results_page(
    ...
    with coaching_tab:
        ...
        with r_col:
            ...
            inj   = max(summary.get("injury_risk_count", 0), summary.get("session_injury_risk_frames", 0))
            falls = max(summary.get("fall_alerts", 0), summary.get("session_fall_alert_frames", 0))
    ...
    with performance_tab:
        ...
        biomech = data.get("biomechanics", {})
        if biomech:
            st.markdown("## Biomechanics Analysis")

            col1, col2 = st.columns(2)
            with col1:
                balance = biomech.get("balance", {})
                st.markdown("### Balance")
                st.write(f"Stability: {balance.get('stability', 0):.2f}")
                st.write(f"Status: {balance.get('status', 'N/A')}")

                timing = biomech.get("timing", {})
                st.markdown("### Timing")
                st.write(f"Contact: {timing.get('contact', False)}")
                st.write(f"Timing: {timing.get('timing', 'N/A')}")

            with col2:
                contact = biomech.get("contact", {})
                st.markdown("### Contact")
                st.write(f"Hit Quality: {contact.get('quality', 'N/A')}")

                kinetic = biomech.get("kinematic_chain", {})
                st.markdown("### Kinematic Chain")
                st.write(f"Efficiency: {kinetic.get('efficiency', 0):.2f}")
                st.write(f"Status: {kinetic.get('status', 'N/A')}")

            st.markdown("## Injury Risk Detection")
            risk = "Low"
            if biomech.get("balance", {}).get("stability", 0) > 60:
                risk = "High (Unstable posture)"
            elif biomech.get("kinematic_chain", {}).get("efficiency", 0) < 20:
                risk = "Medium (Poor energy transfer)"
            st.write(f"Risk Level: {risk}")

    with health_tab:
        render_health_tab(data, summary, sport_profile, performance_metrics, bad_frames, snippets, pose_summary, primary_posture)
...
_DEFAULTS: dict = {
    ...
    "dashboard_history": [],
    "_hist_key": None,
    "_pending_selected_session_id": None,
}
...
pending_session_id = st.session_state.get("_pending_selected_session_id")
if pending_session_id is not None:
    target_session_id = pending_session_id
    if target_session_id != LATEST_SESSION_OPTION and target_session_id not in session_options:
        target_session_id = _latest_session_id_for_sport(active_sport, workspace_records) if active_sport else LATEST_SESSION_OPTION
    st.session_state["selected_session_id"] = target_session_id
    st.session_state["_pending_selected_session_id"] = None
...
job_state = refresh_job_state(load_job_state())
snapshot = _load_monitor_snapshot(
    active_sport,
    st.session_state.get("selected_session_id", LATEST_SESSION_OPTION),
    job_state,
)
data = snapshot["data"]
summary = snapshot["summary"]
sport_profile = snapshot["sport_profile"]
...
if (
    _page in {"Monitor", "Results"}
    and st.session_state.get("active_sport")
    and not st.session_state.get("analysis_started", False)
    and not _is_running(job_state)
):
    _page = "Analyze"
    st.session_state["app_page"] = "Analyze"
